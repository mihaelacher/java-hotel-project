...
public class RoomsPanel extends JPanel {
	...
	public void refreshTable(JTable table, String tabDB) {
		...
		try {
			// Get table data with help of joins in order to avoid using id values from referenced tables
			state = conn.prepareStatement(
					"select rooms.id, rooms.room_number, room_type.name, room_status.status from rooms join room_type on room_type.id = rooms.room_type_id join room_status on room_status.id = rooms.room_status_id;");
	...
	public void setRoomStatusComboValues() {
		// Remove items in order to load new data in case in the meantime something has changed in DB
	...
	class MouseAction implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getClickCount() > 1) {
				roomNubmerTF.setText(table.getValueAt(row, 1).toString());
				...
				// Loop through all combo items, when match is found set combo value and break from the loop
